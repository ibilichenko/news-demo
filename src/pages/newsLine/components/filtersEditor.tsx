...
function deleteCategory(paramsStr: string, chosenParam: string | null): string {
  ...
  if (chosenParam) {
    if (parsedParamsStr.indexOf(chosenParam) === 0) {
      const categoryToSearch = paramsStr.replace(`${chosenParam}${parsedParamsStr.length > 1 ? ',' : ''}`, '');
  ...
  if (data.length > 0) {
    return (
      <div className={styles.filtersContainer}>
        <div className={styles.cancelBtn} onClick={() => history.push(routePaths.newsLine)}>Cancel</div>
                ...
                >
                  <button
                    ...
                    onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
                      history.push(`${routePaths.newsLine}?language=${data[0]}${deleteCategory(data[1], (e.target as HTMLButtonElement).id)}`)
                    }}
