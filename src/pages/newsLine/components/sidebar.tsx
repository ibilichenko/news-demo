...
const Sidebar = ({ data }: { data: string[] }) => {
    ...
    return (
      ...
      <div className={styles.firstBlock}>
            ...
            <div className={styles.sidebar}>
                <Checkbox.Group
                    ...
                    onChange={(value) => {
                        ...
                        history.push(`${routePaths.newsLine}${data[0] ? `?language=${data[0]}` : '?language=en'}${chosenCategories.length > 0 ? `&${prepareUrl('category', value)}` : ''}`);
                    }}
                >
                    <ul>
                        {
            ...
            <div className={styles.title}><p>languages</p><b>{Object.keys(languages).length}</b></div>

            <Radio.Group
                value={data[0]}
                onChange={(event) => {
                    chosenLanguage = event.target.value;
                    history.push(`${routePaths.newsLine}?language=${chosenLanguage ? `${chosenLanguage}` : 'en'}${data[1] ? `&category=${data[1]}` : ''}`);
                }}
            >
                <ul>
                    {
                        Object.keys(languages)?.map((language, index) => {
                            return (
                                <li style={{ listStyleType: "none", marginLeft: 0 }} key={index}><Radio key={index.toString()} value={languages[language]}>{language}</Radio></li>
                            )
                        })
                    }
                </ul>
